...
def extension_for_format(fmt: str) -> str:
    f = normalize_format(fmt)
    return "yml" if f == "yaml" else ("jsonl" if f == "jsonl" else f)
...
def flatten_record_for_csv(
    ...
    *,
    ...
    key_set: set[str] = set()
    return _flatten_object(record, delim, nested_as_json, key_set)


def csv_header_line(headers: list[str]) -> str:
    return ",".join(csv_escape(h) for h in headers)
